...
### Custom SFT, Evaluation, & Serving Sandbox

We have added a custom developer assistant dataset, a unified training and experiment tracking runner, a custom evaluation pipeline, and a FastAPI server with an embedded, interactive chat sandbox interface.

#### 1. Custom Dataset Setup
A high-quality Supervised Fine-Tuning (SFT) dataset containing developer queries, training strategies, and llama-factory parameters has been created at [data/custom_assistant.json](file:///Users/padmanabhjagdishwanikar/Downloads/Llama%20Factory/data/custom_assistant.json). It is registered in [data/dataset_info.json](file:///Users/padmanabhjagdishwanikar/Downloads/Llama%20Factory/data/dataset_info.json) under the key `custom_assistant`.

You can initiate SFT training using this dataset with the custom config file [examples/train_lora/custom_sft.yaml](file:///Users/padmanabhjagdishwanikar/Downloads/Llama%20Factory/examples/train_lora/custom_sft.yaml):
```bash
llamafactory-cli train examples/train_lora/custom_sft.yaml
```

#### 2. Unified SFT & W&B Experiment Tracking
The script [scripts/train_with_wandb.py](file:///Users/padmanabhjagdishwanikar/Downloads/Llama%20Factory/scripts/train_with_wandb.py) handles the entire end-to-end flow: it runs training with LLaMA-Factory, logs loss curves to W&B, and automatically runs the evaluation pipeline upon completion to upload evaluation tables.

To run training & evaluation with W&B logging in dry-run/simulation mode:
```bash
WANDB_MODE=offline python3 scripts/train_with_wandb.py --dry_run
```

#### 3. Automated Evaluation Pipeline
The evaluation pipeline at [scripts/custom_eval.py](file:///Users/padmanabhjagdishwanikar/Downloads/Llama%20Factory/scripts/custom_eval.py) computes NLP metrics (BLEU-4, ROUGE-1, ROUGE-2, ROUGE-L, and Exact Match) between model responses and target references. It also uploads a `wandb.Table` containing instructions, references, model outputs, and metrics side-by-side to W&B.

To run evaluation independently:
```bash
python3 scripts/custom_eval.py --model_name_or_path /path/to/checkpoint --eval_dataset data/custom_assistant.json
```

#### 4. FastAPI Serving & Web UI Sandbox
The FastAPI server at [src/llamafactory/api/custom_server.py](file:///Users/padmanabhjagdishwanikar/Downloads/Llama%20Factory/src/llamafactory/api/custom_server.py) serves the model and hosts an interactive Web Chat Sandbox at the root (`/`) endpoint.
The interface is styled with premium glassmorphism, responsive bubble chats, system metrics monitors, and hyperparameters configuration sliders.

To launch the FastAPI server:
```bash
python3 src/llamafactory/api/custom_server.py --port 8000
```
Then visit `http://localhost:8000/` in your browser to interact with the model in the graphic sandbox UI.
